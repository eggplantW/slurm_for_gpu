			     ...
			     gres_state_t *gres_ptr)
{
	...
	//added by xuewu
	if(gres_data->timeInUse == NULL)
	{
		gres_data->timeInUse = (time_t *)malloc(sizeof(time_t) * gres_data->gres_cnt_avail);
		int i;
		for(i = 0;i < gres_data->gres_cnt_avail;i ++)
			gres_data->timeInUse[i] = 0;
	}
	return rc;
}
...
static void *_job_state_dup(void *gres_data)
{
	...
	if(gres_ptr->addr)
	{
		new_gres_ptr->addr = xmalloc(sizeof(slurm_addr_t) * gres_ptr->node_cnt);
		for (i=0; i<gres_ptr->node_cnt; i++)
			memcpy(&new_gres_ptr->addr[i],&gres_ptr->addr[i],sizeof(slurm_addr_t));
	}
	//wxgresend 9
	return new_gres_ptr;
}
				      ...
				      uint16_t protocol_version)
{
	...
	while ((gres_ptr = (gres_state_t *) list_next(gres_iter))) {
		...
		if (protocol_version >= SLURM_15_08_PROTOCOL_VERSION) {
			...
			//wxgresstart 10
			if (gres_job_ptr->addr)
			{
				pack8((uint8_t) 1, buffer);
				for (i = 0; i < gres_job_ptr->node_cnt; i++)
					pack32(gres_job_ptr->addr[i].sin_addr.s_addr,buffer);
			}
			else
				pack8((uint8_t) 0, buffer);
			//wxgresend 10
					...
					uint16_t protocol_version)
{
	...
	while ((rc == SLURM_SUCCESS) && (rec_cnt)) {
		...
		if (protocol_version >= SLURM_15_08_PROTOCOL_VERSION) {
			...
			if(has_more)
			{
				gres_job_ptr->addr = xmalloc(sizeof(slurm_addr_t)*gres_job_ptr->node_cnt);
				for(i = 0;i < gres_job_ptr->node_cnt;i ++)
					safe_unpack32(&gres_job_ptr->addr[i].sin_addr.s_addr,buffer);
			}
			//wxgresend 11
			safe_unpack8(&has_more, buffer);
			if (has_more) {
...
//wxgresstart gres
extern uint64_t gpu_plugin_job_get_gres_cnt_alloc(List job_gres_list,char *name)
{
	gres_state_t *gres_ptr = NULL;
	gres_job_state_t *gres_job_ptr = NULL;
	ListIterator gres_iter;
	int i;
	if(job_gres_list == NULL || name == NULL)
		return 0;
	slurm_mutex_lock(&gres_context_lock);
	for (i = 0; i < gres_context_cnt; i ++) {
		if (strcmp(gres_context[i].gres_name, name))
	...
	if(gres_job_ptr == NULL)
		return 0;
	return gres_job_ptr->gres_cnt_alloc;
}
extern uint64_t gpu_plugin_node_get_gres_cnt(List node_gres_list,bool test_only,char *name)
{
	gres_state_t *gres_ptr = NULL;
	gres_node_state_t *gres_node_ptr = NULL;
	ListIterator gres_iter;
	int i;
	if(node_gres_list == NULL || name == NULL)
		return 0;
	slurm_mutex_lock(&gres_context_lock);
	for (i = 0; i < gres_context_cnt; i ++) {
		if (strcmp(gres_context[i].gres_name, name))
	...
	slurm_mutex_unlock(&gres_context_lock);
	if(gres_node_ptr == NULL)
		return 0;
	if(test_only)
		return gres_node_ptr->gres_cnt_avail;
	if((gres_node_ptr->gres_cnt_avail - gres_node_ptr->gres_cnt_alloc) < 0)
		return 0;
	return (gres_node_ptr->gres_cnt_avail - gres_node_ptr->gres_cnt_alloc);
}
extern void gpu_plugin_job_set_gpu_bitmap(List job_gres_list,bitstr_t* bitmap,char *name)
{
	gres_state_t *gres_ptr = NULL;
	gres_job_state_t *gres_job_ptr = NULL;
	ListIterator gres_iter;
	...
	slurm_mutex_unlock(&gres_context_lock);
	if(gres_job_ptr->gpu_bitmap != NULL)
		bit_free(gres_job_ptr->gpu_bitmap);
	gres_job_ptr->gpu_bitmap = bit_copy(bitmap);
	return;
}
extern bitstr_t* gpu_plugin_job_get_gpu_bitmap(List job_gres_list,char *name)
{
	gres_state_t *gres_ptr = NULL;
	gres_job_state_t *gres_job_ptr = NULL;
	ListIterator gres_iter;
	...
	slurm_mutex_unlock(&gres_context_lock);
	return gres_job_ptr->gpu_bitmap;
}
extern void gpu_plugin_job_gpu_bitmap_clear(List job_gres_list,int index,char *name)
{
	gres_state_t *gres_ptr = NULL;
	gres_job_state_t *gres_job_ptr = NULL;
	ListIterator gres_iter;
	...
	slurm_mutex_unlock(&gres_context_lock);
	if(gres_job_ptr->gpu_bitmap)
	{
		if(bit_test(gres_job_ptr->gpu_bitmap,index))
			bit_clear(gres_job_ptr->gpu_bitmap,index);
	}
	return;
}
extern void gpu_plugin_job_gpu_bitmap_set(List job_gres_list,int index,char *name)
{
	gres_state_t *gres_ptr = NULL;
	gres_job_state_t *gres_job_ptr = NULL;
	ListIterator gres_iter;
	...
	{
		if(!bit_test(gres_job_ptr->gpu_bitmap,index))
			bit_set(gres_job_ptr->gpu_bitmap,index);
	}
	return;
}
extern void gpu_plugin_job_set_node_cnt(List job_gres_list,uint32_t node_cnt,char *name)
{
	gres_state_t *gres_ptr = NULL;
	gres_job_state_t *gres_job_ptr = NULL;
	ListIterator gres_iter;
	...
	slurm_mutex_unlock(&gres_context_lock);
	gres_job_ptr->node_cnt = node_cnt;
	if(gres_job_ptr->gres_bit_alloc != NULL)
		xfree(gres_job_ptr->gres_bit_alloc);
	gres_job_ptr->gres_bit_alloc = xmalloc(sizeof(bitstr_t *) * node_cnt);
	if(gres_job_ptr->addr != NULL)
		xfree(gres_job_ptr->addr);
	gres_job_ptr->addr = xmalloc(sizeof(slurm_addr_t) * node_cnt);
	return;
}
extern void gpu_plugin_job_alloc(List job_gres_list,List node_gres_list,int index,uint64_t *req,slurm_addr_t addr,char *name)
{
	gres_state_t *job_gres_ptr = NULL,*node_gres_ptr = NULL;
	gres_job_state_t *gres_job_ptr = NULL;
	gres_node_state_t *gres_node_ptr = NULL;
	ListIterator job_gres_iter,node_gres_iter;
	int i;
	if(job_gres_list == NULL || node_gres_list == NULL || name == NULL)
			return;
	slurm_mutex_lock(&gres_context_lock);
	for (i = 0; i < gres_context_cnt; i ++) {
		if (strcmp(gres_context[i].gres_name, name))
		...
		while ((job_gres_ptr = (gres_state_t *)list_next(job_gres_iter))) {
			if (job_gres_ptr->plugin_id != gres_context[i].plugin_id)
				continue;
			gres_job_ptr = (gres_job_state_t*)job_gres_ptr->gres_data;
			break;
		}
		list_iterator_destroy(job_gres_iter);
		node_gres_iter = list_iterator_create(node_gres_list);
		while ((node_gres_ptr = (gres_state_t *)list_next(node_gres_iter))) {
			if (node_gres_ptr->plugin_id != gres_context[i].plugin_id)
				continue;
			gres_node_ptr = (gres_node_state_t*)node_gres_ptr->gres_data;
			break;
		}
		list_iterator_destroy(node_gres_iter);
		break;
	...
	slurm_mutex_unlock(&gres_context_lock);
	if(gres_job_ptr->gres_bit_alloc)
		FREE_NULL_BITMAP(gres_job_ptr->gres_bit_alloc[index]);
	gres_job_ptr->gres_bit_alloc[index] = bit_alloc(gres_node_ptr->gres_cnt_avail);
	for(i = 0;i < gres_node_ptr->gres_cnt_avail;i ++)
	{
		if(*req <= 0)
			break;
		if(bit_test(gres_node_ptr->gres_bit_alloc,i))
			continue;
		bit_set(gres_node_ptr->gres_bit_alloc,i);
		bit_set(gres_job_ptr->gres_bit_alloc[index],i);
		(*req) --;
		gres_node_ptr->gres_cnt_alloc ++;
	}
	memcpy(&gres_job_ptr->addr[index],&addr,sizeof(slurm_addr_t));
	return;
}
extern void gpu_plugin_job_dealloc(List job_gres_list,List node_gres_list,int index,char *name,time_t time)
{
	gres_state_t *job_gres_ptr = NULL,*node_gres_ptr = NULL;
	gres_job_state_t *gres_job_ptr = NULL;
	gres_node_state_t *gres_node_ptr = NULL;
	...
	slurm_mutex_unlock(&gres_context_lock);
	for(i = 0;i < bit_size(gres_job_ptr->gres_bit_alloc[index]);i ++)
	{
		if(bit_test(gres_job_ptr->gres_bit_alloc[index],i))
		{
			bit_clear(gres_node_ptr->gres_bit_alloc,i);
			if(gres_node_ptr->timeInUse != NULL)
			{
				gres_node_ptr->timeInUse[i] += time;
				info("gpu %d timeInUse=%d\n",i,gres_node_ptr->timeInUse[i]);
			}
			gres_node_ptr->gres_cnt_alloc --;
		}
	}
...
extern uint32_t gpu_plugin_job_get_node_cnt(List job_gres_list,char *name)
{
	gres_state_t *gres_ptr = NULL;
	gres_job_state_t *gres_job_ptr = NULL;
	ListIterator gres_iter;
	...
	if(gres_job_ptr == NULL)
		return 0;
	return gres_job_ptr->node_cnt;
}
extern bitstr_t** gpu_plugin_job_get_gres_bit_alloc(List job_gres_list,char *name)
{
	gres_state_t *gres_ptr = NULL;
	gres_job_state_t *gres_job_ptr = NULL;
	ListIterator gres_iter;
	...
	if(gres_job_ptr == NULL)
		return NULL;
	return gres_job_ptr->gres_bit_alloc;
}
extern slurm_addr_t * gpu_plugin_job_get_addr(List job_gres_list,char *name)
{
	gres_state_t *gres_ptr = NULL;
	gres_job_state_t *gres_job_ptr = NULL;
	ListIterator gres_iter;
