			...
			char *node_name, time_t t)
{
	...
	if (node_gres_ptr->gres_bit_alloc && job_gres_ptr->gres_bit_alloc &&
		...
		for (i = 0; i < len; i++) {
			...
			//added by xuewu
			info("i=%d\n",i);
			if(node_gres_ptr->timeInUse != NULL)
			{
				node_gres_ptr->timeInUse[i] =node_gres_ptr->timeInUse[i] + t;
				info("gpu %d timeInUse=%d\n",i,node_gres_ptr->timeInUse[i]);
			}
