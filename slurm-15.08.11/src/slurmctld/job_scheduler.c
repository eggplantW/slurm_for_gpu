...
extern void launch_job(struct job_record *job_ptr)
{
	...
	//added by xuewu
	static time_t totalWaitTime = 0;
	time_t jobStartTime = time(NULL);
	job_ptr->startTime = jobStartTime;
	totalWaitTime = totalWaitTime + (job_ptr->startTime - job_ptr->submitTime);
	info("job start: job_id = %d\ttime = %d\n",job_ptr->job_id,jobStartTime);
	info("job wait time = %d\n",totalWaitTime);
