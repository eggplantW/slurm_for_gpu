...
{
	uint16_t cpus;
	uint32_t avail_mem, req_mem, /*wxselect 4 gres_cores, gres_cpus wxselect 4,*/ cpus_per_core;
	int core_start_bit, core_end_bit, cpu_alloc_size, i;
	struct node_record *node_ptr = node_record_table_ptr + node_i;
	//List gres_list;//wxselect 4
			      ...
			      bitstr_t *exc_core_bitmap, bool qos_preemptor)
{
	...
	uint32_t i, j, free_mem, /*wxselect 2gres_cpus, gres_cores wxselect 2,*/ min_mem;
	int core_start_bit, core_end_bit, cpus_per_core;
	//List gres_list;//wxselect 2
	int i_first, i_last;
			...
			bool qos_preemptor, bool preempt_mode)
{
	...
	uint16_t *cpu_count;
	int i, first, last;
	List gres_list;//wxselect 3
	uint64_t job_gpu_req_cnt = 0,node_gpu_cnt = 0,node_gpu_total_cnt = 0;//wxselect 3
	bitstr_t *gpubitmap = NULL;//wxselect 3
	if (gang_mode == -1) {
	...
	{
		gpubitmap = bit_alloc(cr_node_cnt);
		...
		for(i = 0;i < cr_node_cnt;i ++)
		{
			...
			if(node_usage[i].gres_list)
				...
				gres_list = (select_node_record[i].node_ptr)->gres_list;
			node_gpu_cnt = gpu_plugin_node_get_gres_cnt(gres_list,test_only,"gpu");
			if(node_gpu_cnt > 0)
			{
				bit_set(gpubitmap,i);
				node_gpu_total_cnt +=  node_gpu_cnt;
			}
		}
		if(node_gpu_total_cnt < job_gpu_req_cnt)
		{
			bit_free(gpubitmap);
			return SLURM_ERROR;
		}
		if(!test_only)
			gpu_plugin_job_set_gpu_bitmap(job_ptr->gres_list,gpubitmap,"gpu");
		bit_free(gpubitmap);
	}
	//wxselectend 3
