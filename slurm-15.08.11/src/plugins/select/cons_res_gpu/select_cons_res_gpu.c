/*****************************************************************************\
 *  select_cons_res_gpu.c - node selection plugin supporting consumable
 *  resources policies.
 *****************************************************************************\
 *
...
#include "select_cons_res_gpu.h"
#include "../cons_res_gpu/dist_tasks.h"
#include "../cons_res_gpu/job_test.h"
...
const char plugin_type[] = "select/cons_res_gpu";//wxselect 1
const uint32_t plugin_id      = 109;//wxselect 1
...
static void _create_part_data(void)
{
	...
	if (!num_parts)
		return;
	info("cons_res_gpu: preparing for %d partitions", num_parts);
			       ...
			       struct job_record *job_ptr)
{
	...
	debug3("cons_res_gpu: build_row_bitmaps reshuffling %u jobs", num_jobs);
	...
	if (j < num_jobs) {
		...
		debug3("cons_res_gpu: build_row_bitmap: dangling job found");
 ...
 * if action = 1 then add memory(adding suspended job)//wxselect
 * if action = 2 then only add cores + GRES(suspended job is resumed)//wxselect
 */
static int _add_job_to_res(struct job_record *job_ptr, int action)
{
	...
	uint64_t job_gpu_req_cnt = 0,node_gpu_cnt = 0,tmp_gpu_cnt = 0;//wxselect 5
	bitstr_t *gpubitmap = NULL;//wxselect 5
	uint64_t *gpu_cnt = NULL;//wxselect 5
	uint32_t node_cnt = 0,select_cnt = 0;//wxselect 5
	...
	debug3("cons_res_gpu: _add_job_to_res: job %u act %d ", job_ptr->job_id,
	       action);
...
//wxselectend 5.1
		if (action != 2) {
			...
			if ((select_node_usage[i].alloc_memory >
			     select_node_record[i].real_memory)) {
				error("cons_res_gpu: node %s memory is "
...
//wxselectstart 5.2
	if(action != 1)
	{
		...
		{
			job_gpu_req_cnt = gpu_plugin_job_get_gres_cnt_alloc(job_ptr->gres_list,"gpu");
			gpubitmap = gpu_plugin_job_get_gpu_bitmap(job_ptr->gres_list,"gpu");
			if(gpubitmap != NULL)
				node_cnt = bit_size(gpubitmap);
			gpu_cnt = xmalloc(sizeof(uint64_t) * node_cnt);
			for(i = 0;i < node_cnt;i ++)
			{
				if(bit_test(gpubitmap,i))
				{
					node_ptr = select_node_record[i].node_ptr;
					if (select_node_usage[i].gres_list)
						gres_list = select_node_usage[i].gres_list;
						...
						gres_list = node_ptr->gres_list;
					gpu_cnt[i] = gpu_plugin_node_get_gres_cnt(gres_list,false,"gpu");
					gpu_plugin_job_gpu_bitmap_clear(job_ptr->gres_list,i,"gpu");
				}
				else
					gpu_cnt[i] = 0;
			}
			for(i = 0;i < node_cnt;i ++)
			{
				if(gpu_cnt[i] > 0)
				{
					if(tmp_gpu_cnt >=job_gpu_req_cnt)
						break;
					else
					{
						tmp_gpu_cnt += gpu_cnt[i];
						select_cnt ++;
					}
				}
			}
			gpu_plugin_job_set_node_cnt(job_ptr->gres_list,select_cnt,"gpu");
			for(i = 0,n = -1;i < node_cnt && job_gpu_req_cnt > 0;i ++)
			{
				if(gpu_cnt[i])
				{
					n ++;
					gpu_plugin_job_gpu_bitmap_set(job_ptr->gres_list,i,"gpu");
					node_ptr = select_node_record[i].node_ptr;
					if (select_node_usage[i].gres_list)
						gres_list = select_node_usage[i].gres_list;
					else
						gres_list = node_ptr->gres_list;
					gpu_plugin_job_alloc(job_ptr->gres_list,gres_list,n,&job_gpu_req_cnt,node_ptr->slurm_addr,"gpu");
				}
			}
			xfree(gpu_cnt);
		}
...
//wxselectend 5.2
	...
	if (action != 1) {
		...
		if (!p_ptr) {
			error("cons_res_gpu: could not find cr partition %s",
		...
		for (i = 0; i < p_ptr->num_rows; i++) {
			if (!_can_job_fit_in_row(job, &(p_ptr->row[i])))
				continue;
			debug3("cons_res_gpu: adding job %u to part %s row %u",
		...
		if (i >= p_ptr->num_rows) {
			...
			error("cons_res_gpu: job overflow: "
		       ...
		       struct job_record *to_job_ptr)
{
	...
	if (from_job_ptr->job_id == to_job_ptr->job_id) {
		error("select/cons_res_gpu: attempt to merge job %u with self",
		      from_job_ptr->job_id);
		return SLURM_ERROR;
	}
	...
	if ((from_job_resrcs_ptr == NULL) ||
		...
		error("select/cons_res_gpu: job %u lacks a job_resources struct",
		      from_job_ptr->job_id);
		return SLURM_ERROR;
	}
 ...
 * if action = 1 then subtract memory(suspended job was terminated)//wxselect
 * if action = 2 then only subtract cores  + GRES (job is suspended)//wxselect
 *
 */
static int _rm_job_from_res(struct part_res_record *part_record_ptr,
			    struct node_use_record *node_usage,
			    struct job_record *job_ptr, int action)
{
	...
	int i, n;
	List gres_list;
	bitstr_t *gpubitmap;//wxselect 6
	if (select_state_initializing) {
		/* Ignore job removal until select/cons_res_gpu data structures
	...
	debug3("cons_res_gpu: _rm_job_from_res: job %u action %d", job_ptr->job_id,
	       action);
	if (select_debug_flags & DEBUG_FLAG_SELECT_TYPE)
		_dump_job_res(job);
...
//wxselectend 6.1
	...
	if(action != 1)
	{
		if(job_ptr->gres_list)
		{
			...
			if(gpubitmap)
			{
				first_bit = bit_ffs(gpubitmap);
				if (first_bit == -1)
					last_bit = -2;
				else
					last_bit =  bit_fls(gpubitmap);
				for(i = first_bit,n = -1;i <= last_bit;i ++)
				{
					if(bit_test(gpubitmap,i))
					{
						...
						if (node_usage[i].gres_list)
							...
							gres_list = node_ptr->gres_list;
						//added by xuewu
						info("gpu time on node %d :\n",i);
						//gpu_plugin_job_dealloc(job_ptr->gres_list,gres_list,n,"gpu");
						gpu_plugin_job_dealloc(job_ptr->gres_list,gres_list,n,"gpu",job_ptr->endTime - job_ptr->startTime);
					}
				}
			}
		}
	...
	if (action != 1) {
		...
		if (!job_ptr->part_ptr) {
			error("cons_res_gpu: removed job %u does not have a "
			      "partition assigned",
			      job_ptr->job_id);
			return SLURM_ERROR;
		}
		...
		if (!p_ptr) {
			error("cons_res_gpu: removed job %u could not find part %s",
		...
		if (n) {
			...
			for (i = first_bit, n = -1; i <= last_bit; i++) {
				...
				if (node_usage[i].node_state >=
					...
					error("cons_res_gpu:_rm_job_from_res: "
				 ...
				 struct node_record *node_ptr)
{
	...
	if (!job || !job->core_bitmap) {
		error("%s: select/cons_res_gpu: job %u has no job_resrcs info",
		      __func__, job_ptr->job_id);
		return SLURM_ERROR;
	}

	debug3("cons_res_gpu: _rm_job_from_one_node: job %u node %s",
	       job_ptr->job_id, node_ptr->name);
	if (select_debug_flags & DEBUG_FLAG_SELECT_TYPE)
		_dump_job_res(job);
	...
	for (i = first_bit, n = -1; i <= last_bit; i++) {
		...
		if (node_usage[i].alloc_memory < job->memory_allocated[n]) {
			error("cons_res_gpu: node %s memory is underallocated "
			      "(%u-%u) for job %u",
	...
	if (!job_ptr->part_ptr) {
		error("cons_res_gpu: removed job %u does not have a partition "
	...
	for (i = 0; i < p_ptr->num_rows; i++) {
		...
		for (j = 0; j < p_ptr->row[i].num_jobs; j++) {
			if (p_ptr->row[i].job_list[j] != job)
				continue;
			debug3("cons_res_gpu: found job %u in part %s row %u",
			       job_ptr->job_id, p_ptr->part_ptr->name, i);
	...
	if (n == 0) {
		error("cons_res_gpu: could not find job %u in partition %s",
	...
	if (node_usage[node_inx].node_state >= job->node_req) {
		...
		error("cons_res_gpu:_rm_job_from_one_node: node_state miscount");
		      ...
		      uint32_t req_nodes, uint16_t job_node_req)
{
	...
	if (job_ptr->part_ptr->cr_type) {
		if ((cr_type & CR_SOCKET) || (cr_type & CR_CORE)) {
			...
			info("cons_res_gpu: Can't use Partition SelectType unless "
 ...
 * the select/cons_res_gpu plugin data structures. */
		...
		while (more_jobs) {
			...
			while (true) {
				...
				if (overlap == 0)  /* job has no usable nodes */
					continue;  /* skip it */
				debug2("cons_res_gpu: _will_run_test, job %u: overlap=%d",
...
extern int select_p_node_init(struct node_record *node_ptr, int node_cnt)
{
	...
	info("cons_res_gpu: select_p_node_init");
			     ...
			     bitstr_t *exc_core_bitmap)
{
	...
	if (select_debug_flags & DEBUG_FLAG_SELECT_TYPE) {
		info("cons_res_gpu: select_p_job_test: job %u node_req %u mode %d",
		     job_ptr->job_id, job_node_req, mode);
		info("cons_res_gpu: select_p_job_test: min_n %u max_n %u req_n %u "
		     "avail_n %u",
...
extern int select_p_reconfigure(void)
{
	...
	info("cons_res_gpu: select_p_reconfigure");
	select_debug_flags = slurm_get_debug_flags();
