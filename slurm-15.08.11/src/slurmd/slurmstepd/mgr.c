...
//wxstepdstart 2
static char* _make_gpu_script(batch_job_launch_msg_t *msg,char *path);
//wxstepdend 2
/*
...
batch_finish(stepd_step_rec_t *job, int rc)
{
	...
	if(strcasecmp(type, "select/cons_res_gpu"))//wxstepdend 3.1
	 ...
	 * batch job. */	
	//wxstepdstart 3.2
	if(strcasecmp(type, "select/cons_res_gpu"))//wxstepdend 3.2
...
mgr_launch_batch_job_setup(batch_job_launch_msg_t *msg, slurm_addr_t *cli)
{
	...
	char *type = NULL;
	type = slurm_get_select_type();
	if(!strcasecmp(type, "select/cons_res_gpu") && get_cred_list(msg->cred) != NULL)
	{
		if ((job->argv[0] = _make_gpu_script(msg, job->batchdir)) == NULL)
				goto cleanup;
	}
	else	//wxstepdend 1
...
static char* _make_gpu_script(batch_job_launch_msg_t *msg,char *path)
{
	FILE *fp = NULL,*fphf = NULL,*fpgpuhf = NULL;
	char script[MAXPATHLEN],hf[MAXPATHLEN],gpuhf[MAXPATHLEN];
	List gres_list = get_cred_list(msg->cred);
	uint64_t gres_cnt_alloc;
	uint32_t node_cnt,n = 0;
	bitstr_t **gres_bit_alloc;
	slurm_addr_t *addr;
	char *buf = NULL,*bufleft,*tmp;
	int buf_size = BUFSIZ;//8192
	...
	if (msg->script == NULL)
	{
		error("_make_gpu_script: called with NULL script");
		return NULL;
	}
	if(gres_list == NULL)
	{
		error("_make_gpu_script: called with NULL job_gres_list");
		return NULL;
	}
	snprintf(script, sizeof(script), "%s/%s", path, "slurm_script");
	snprintf(hf, sizeof(script), "%s/%s", path, "slurm_hostfile");
	snprintf(gpuhf, sizeof(script), "%s/%s", path, "slurm_gpu_hostfile");
...
againhf:
	if ((fphf = safeopen(hf, "w", SAFEOPEN_CREATE_ONLY)) == NULL)
	{
		if ((errno != EEXIST) || (unlink(hf) < 0))
		{
			error("couldn't open `%s': %m", hf);
			goto error;
		}
		goto againhf;
	}
againgpuhf:
	if ((fpgpuhf = safeopen(gpuhf, "w", SAFEOPEN_CREATE_ONLY)) == NULL)
	{
		if ((errno != EEXIST) || (unlink(gpuhf) < 0))
		{
			error("couldn't open `%s': %m", gpuhf);
			goto error;
		}
		goto againgpuhf;
	}
	host_list = hostlist_create(msg->nodes);
	i = 0;
	j = 0;
	n = 0;
	total_nodes = msg->cpu_count_reps[i];
	while ((node_name = hostlist_shift(host_list)))
	{
		slurm_addr_t tmp_addr;
		slurm_conf_get_addr(node_name,&tmp_addr);
		char addrbuf[INET_ADDRSTRLEN];
		inet_ntop(AF_INET,&tmp_addr.sin_addr,addrbuf,INET_ADDRSTRLEN);
		if(j >= total_nodes)
		{
			i ++;
			total_nodes += msg->cpu_count_reps[i];
		}
		fprintf(fphf,"%s:%d\n",addrbuf,msg->cpus_per_node[i]);
		n += msg->cpus_per_node[i];
		j ++;
		free(node_name);
	}
	...
	strcpy(buf,msg->script);
	tmp = strtok_r(buf, "\n ", &bufleft);
	while(tmp != NULL)
	{
		if(!strcmp(tmp,"sgpu"))
		{
			fprintf(fp,"\nexport LD_LIBRARY_PATH=/home/run/cuda-workplace/cudart_remote/src/:$LD_LIBRARY_PATH\n/home/run/wx-workplace/mvapich2-2.2rc2/bin/mpiexec "
					"-genv MV2_ENABLE_AFFINITY 0 "
					"-genv MV2_SUPPORT_DPM 1 "
					"-genv MV2_SMP_USE_CMA 0 "
					"-genv GC_DEVICE_HOSTS \"%s\" "
					"-genv GC_DEVICE_LIST \"",gpuhf);
			gres_cnt_alloc = gpu_plugin_job_get_gres_cnt_alloc(gres_list,"gpu");
			node_cnt = gpu_plugin_job_get_node_cnt(gres_list,"gpu");
			gres_bit_alloc = gpu_plugin_job_get_gres_bit_alloc(gres_list,"gpu");
			addr = gpu_plugin_job_get_addr(gres_list,"gpu");
			if(gres_bit_alloc != NULL && addr != NULL)
			{
				for(i = 0;i < node_cnt;i ++)
				{
					if(gres_bit_alloc[i] == NULL)
						continue;
					...
					char addrbuf[INET_ADDRSTRLEN];
					inet_ntop(AF_INET,&addr[i].sin_addr,addrbuf,INET_ADDRSTRLEN);
					fprintf(fpgpuhf,"%s:%d\n",addrbuf,bit_set_count(gres_bit_alloc[i]));
				}
			}
			fprintf(fp,"\" -f \"%s\" -n %d -env CUDA_VISIBLE_DEVICES \"\" ",hf,n);
		}
		else
			fprintf(fp,"%s ",tmp);
		tmp = strtok_r(NULL,"\n ",&bufleft);
	...
	if (fclose(fp) < 0 || fclose(fphf) || fclose(fpgpuhf))
		error("fclose: %m");
	if (chown(script, (uid_t) msg->uid, (gid_t) -1) < 0)
	{
		error("chown(%s): %m", path);
