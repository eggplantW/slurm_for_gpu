...
exec_task(stepd_step_rec_t *job, int i)
{
	...
	char tmpId[10],tmpdevice[5];//wx
	char tmpexecPath[256],tmphfPath[256],tmpremoteHostsPath[256],tmpDevice[256],tmpstr[1024];//wx
	int fdexec,fdhf,fdremoteHosts;//wx
	char addrbuf[INET_ADDRSTRLEN + 5];//wx
	bitstr_t *nodebitmap;//wx
	slurm_addr_t *slurm_addr;//wx
	uint32_t node_cnt;		//wx
	bitstr_t **gres_bit_alloc;//wx
	uint16_t *core_cnt;//wx
	slurm_addr_t *core_node_slurm_addr;//wx
	char *execargv[2];//wx
	...
	*/
	...
	//wx
	sprintf(tmpId,"%"PRIu32"",job->jobid);
	tmpexecPath[0] = 0;
	strcat(tmpexecPath,"/home/run/cuda-workplace/cudart_remote/src/b_");
	strcat(tmpexecPath,tmpId);
	strcat(tmpexecPath,".sh");
	fdexec = creat(tmpexecPath,S_IRUSR|S_IWUSR|S_IXUSR);
	tmphfPath[0] = 0;
	strcat(tmphfPath,"/home/run/cuda-workplace/cudart_remote/src/hf_");
	strcat(tmphfPath,tmpId);
	fdhf = creat(tmphfPath,S_IRUSR|S_IWUSR);
	tmpremoteHostsPath[0] = 0;
	strcat(tmpremoteHostsPath,"/home/run/cuda-workplace/cudart_remote/src/remoteHosts_");
	strcat(tmpremoteHostsPath,tmpId);
	fdremoteHosts = creat(tmpremoteHostsPath,S_IRUSR|S_IWUSR);
	tmpstr[0] = 0;
	tmpDevice[0] = 0;
	strcat(tmpstr,"#!/bin/sh\n/home/run/wx-workplace/mvapich2-2.1/bin/mpiexec -genv MV2_ENABLE_AFFINITY \"0\" -genv MV2_SUPPORT_DPM \"1\" -genv MV2_SMP_USE_CMA 0 -genv GC_DEVICE_LIST \"");
	nodebitmap = gres_plugin_job_get_nodebitmap(job->job_gres_list);
	slurm_addr = gres_plugin_job_get_slurm_addr(job->job_gres_list);//wx
	node_cnt = gres_plugin_job_get_node_cnt(job->job_gres_list);		/* 0 if no_consume */
	gres_bit_alloc = gres_plugin_job_get_gres_bit_alloc(job->job_gres_list);
	core_cnt = gres_plugin_job_get_core_cnt(job->job_gres_list);//wx
	core_node_slurm_addr = gres_plugin_job_get_core_node_slurm_addr(job->job_gres_list);//wx
	for(i = 0;i < node_cnt;i ++)
	{
		if(gres_bit_alloc[i])
		{
			for(j = 0;j < bit_size(gres_bit_alloc[i]);j ++)
			{
				if(!bit_test(gres_bit_alloc[i],j))
					continue;
				if(tmpDevice[0] != 0)
					strcat(tmpDevice," ");
				sprintf(tmpdevice,"%d",j);
				strcat(tmpDevice,tmpdevice);
			}
		}
	}
	strcat(tmpstr,tmpDevice);
	strcat(tmpstr,"\" -genv GC_DEVICE_HOSTS \"");
	strcat(tmpstr,tmpremoteHostsPath);
	strcat(tmpstr,"\" -f ");
	strcat(tmpstr,tmphfPath);
	strcat(tmpstr," -n ");
	j = 0;
	for(i = 0;i < node_cnt;i ++)
	{
	...
	sprintf(tmpdevice,"%d",j);
	strcat(tmpstr,tmpdevice);
	strcat(tmpstr," -env CUDA_VISIBLE_DEVICES \"\" /home/run/cuda-workplace/cudart_remote/src/vectorAdd 0");
	write(fdexec,tmpstr,strlen(tmpstr));
	close(fdexec);
	for(i = 0;i < node_cnt;i ++)
	{
		if(core_cnt[i] > 0)
		{
			inet_ntop(AF_INET,&core_node_slurm_addr[i].sin_addr,addrbuf,INET_ADDRSTRLEN);
			sprintf(tmpdevice,":%d\n",core_cnt[i]);
			strcat(addrbuf,tmpdevice);
			write(fdhf,addrbuf,strlen(addrbuf));
		}
	}
	close(fdhf);
	for(i = 0;i < node_cnt;i ++)
	{
		if(gres_bit_alloc[i])
		{
			inet_ntop(AF_INET,&slurm_addr[i].sin_addr,addrbuf,INET_ADDRSTRLEN);
			sprintf(tmpdevice,":%d\n",bit_set_count(gres_bit_alloc[i]));
			strcat(addrbuf,tmpdevice);
			write(fdremoteHosts,addrbuf,strlen(addrbuf));
		}
	}
	close(fdremoteHosts);
	execargv[0] = tmpexecPath;
	execargv[1] = NULL;
	execve(execargv[0],execargv,NULL);
	//wx
	//execve(task->argv[0], task->argv, job->env);//wx
	saved_errno = errno;
