...
static void *_job_state_dup(void *gres_data)
{
	...
	//wx
	if(gres_ptr->node_gpu_cnt)
	{
		new_gres_ptr->node_gpu_cnt = xmalloc(sizeof(uint64_t)*gres_ptr->node_cnt);
		for(i = 0;i < gres_ptr->node_cnt;i ++)
			new_gres_ptr->node_gpu_cnt[i] = gres_ptr->node_gpu_cnt[i];
	}
	if(gres_ptr->nodebitmap)
		new_gres_ptr->nodebitmap = bit_copy(gres_ptr->nodebitmap);
	if(gres_ptr->slurm_addr)
	{
		new_gres_ptr->slurm_addr = xmalloc(sizeof(slurm_addr_t) * gres_ptr->node_cnt);
		for(i = 0;i < gres_ptr->node_cnt;i ++)
			memcpy(&new_gres_ptr->slurm_addr[i],&gres_ptr->slurm_addr[i],sizeof(slurm_addr_t));
	}
	if(gres_ptr->core_cnt)
	{
		new_gres_ptr->core_cnt = xmalloc(sizeof(uint64_t)*gres_ptr->node_cnt);
		for(i = 0;i < gres_ptr->node_cnt;i ++)
			new_gres_ptr->core_cnt[i] = gres_ptr->core_cnt[i];
	}
	if(gres_ptr->core_node_slurm_addr)
	{
		new_gres_ptr->core_node_slurm_addr = xmalloc(sizeof(slurm_addr_t) * gres_ptr->node_cnt);
		for(i = 0;i < gres_ptr->node_cnt;i ++)
			memcpy(&new_gres_ptr->core_node_slurm_addr[i],&gres_ptr->core_node_slurm_addr[i],sizeof(slurm_addr_t));
	}
	//wx
	return new_gres_ptr;
}
				      ...
				      uint16_t protocol_version)
{
	...
	while ((gres_ptr = (gres_state_t *) list_next(gres_iter))) {
		...
		if (protocol_version >= SLURM_15_08_PROTOCOL_VERSION) {
			...
			//wx
			if(gres_job_ptr->node_gpu_cnt)
			{
				pack8((uint8_t) 1, buffer);
				for(i = 0;i < gres_job_ptr->node_cnt;i ++)
					pack64(gres_job_ptr->node_gpu_cnt[i],buffer);
			}
			else
				pack8((uint8_t) 0, buffer);

			if(gres_job_ptr->nodebitmap)
			{
				pack8((uint8_t) 1, buffer);
				pack_bit_str_hex(gres_job_ptr->nodebitmap,buffer);
			}
			else
				pack8((uint8_t) 0, buffer);

			if(gres_job_ptr->slurm_addr)
			{
				pack8((uint8_t) 1, buffer);
				for(i = 0;i < gres_job_ptr->node_cnt;i++)
					pack32(gres_job_ptr->slurm_addr[i].sin_addr.s_addr,buffer);
			}
			else
				pack8((uint8_t) 0, buffer);

			if(gres_job_ptr->core_cnt)
			{
				pack8((uint8_t) 1,buffer);
				for(i = 0;i < gres_job_ptr->node_cnt;i ++)
					pack16(gres_job_ptr->core_cnt[i],buffer);
			}
			else
				pack8((uint8_t) 0, buffer);

			if(gres_job_ptr->core_node_slurm_addr)
			{
				pack8((uint8_t) 1, buffer);
				for(i = 0;i < gres_job_ptr->node_cnt;i++)
					pack32(gres_job_ptr->core_node_slurm_addr[i].sin_addr.s_addr,buffer);
			}
			else
				pack8((uint8_t) 0, buffer);
			//wx
					...
					uint16_t protocol_version)
{
	...
	while ((rc == SLURM_SUCCESS) && (rec_cnt)) {
		...
		if (protocol_version >= SLURM_15_08_PROTOCOL_VERSION) {
			...
			if(has_more)
			{
				gres_job_ptr->node_gpu_cnt = xmalloc(sizeof(uint64_t) * gres_job_ptr->node_cnt);
				for(i = 0;i < gres_job_ptr->node_cnt;i ++)
					safe_unpack64(&gres_job_ptr->node_gpu_cnt[i],buffer);
			}

			safe_unpack8(&has_more, buffer);
			if(has_more)
			{
				unpack_bit_str_hex(&gres_job_ptr->nodebitmap,buffer);
			}

			safe_unpack8(&has_more, buffer);
			if(has_more)
			{
				gres_job_ptr->slurm_addr = xmalloc(sizeof(slurm_addr_t)*gres_job_ptr->node_cnt);
				for(i = 0;i < gres_job_ptr->node_cnt;i ++)
					safe_unpack32(&gres_job_ptr->slurm_addr[i].sin_addr.s_addr,buffer);
			}

			safe_unpack8(&has_more, buffer);
			if(has_more)
			{
				gres_job_ptr->core_cnt = xmalloc(sizeof(uint64_t) * gres_job_ptr->node_cnt);
				for(i = 0;i < gres_job_ptr->node_cnt;i ++)
					safe_unpack16(&gres_job_ptr->core_cnt[i],buffer);
			}

			safe_unpack8(&has_more, buffer);
			if(has_more)
			{
				gres_job_ptr->core_node_slurm_addr = xmalloc(sizeof(slurm_addr_t)*gres_job_ptr->node_cnt);
				for(i = 0;i < gres_job_ptr->node_cnt;i ++)
					safe_unpack32(&gres_job_ptr->core_node_slurm_addr[i].sin_addr.s_addr,buffer);
			}
			//wx
			/*
			safe_unpack8(&has_more, buffer);
...
//wx
//get the count of gpu needed by job
extern uint64_t gres_plugin_job_req_gpu_cnt(List gres_list)
{
	int i;
	ListIterator gres_iter;
	gres_state_t *gres_ptr;
	gres_job_state_t *job_gres;
	if (gres_list == NULL)
		return 0;
	...
	while ((gres_ptr = (gres_state_t *) list_next(gres_iter)))
	{
		for (i=0; i<gres_context_cnt; i++)
		{
			if (gres_ptr->plugin_id != gres_context[i].plugin_id)
				continue;
			job_gres = (gres_job_state_t *)(gres_ptr->gres_data);
			break;
		}
	}
	list_iterator_destroy(gres_iter);
	slurm_mutex_unlock(&gres_context_lock);
	return job_gres->gres_cnt_alloc;
}
//return node's avail gpu which can allocate to job
extern uint64_t gres_plugin_node_gpu_cnt(List gres_list,bool test_only)
{
	int i;
	ListIterator gres_iter;
	gres_state_t *gres_ptr;
	...
	while ((gres_ptr = (gres_state_t *) list_next(gres_iter)))
	{
		for (i=0; i<gres_context_cnt; i++)
		{
			if (gres_ptr->plugin_id != gres_context[i].plugin_id)
				continue;
			node_gres = (gres_node_state_t *)(gres_ptr->gres_data);
			break;
		}
	}
	list_iterator_destroy(gres_iter);
	slurm_mutex_unlock(&gres_context_lock);
	if(test_only)
		return node_gres->gres_cnt_avail;
	else
		return node_gres->gres_cnt_avail-node_gres->gres_cnt_alloc;
}
extern void gres_plugin_build_job_gpu_cnt(List gres_list,uint64_t *gpu_cnt,uint32_t node_cnt)
{
	int i,j;
	ListIterator gres_iter;
	gres_state_t *gres_ptr;
	...
	if(job_gres->node_gpu_cnt)
		xfree(job_gres->node_gpu_cnt);
	job_gres->node_gpu_cnt = xmalloc(node_cnt * sizeof(uint64_t));
	for(j = 0;j < node_cnt;j ++)
		job_gres->node_gpu_cnt[j] = gpu_cnt[j];
	list_iterator_destroy(gres_iter);
	slurm_mutex_unlock(&gres_context_lock);
	return;
}
extern uint64_t* gres_plugin_job_get_gpu_cnt(List gres_list)
{
	int i;
	ListIterator gres_iter;
	gres_state_t *gres_ptr;
	...
	list_iterator_destroy(gres_iter);
	slurm_mutex_unlock(&gres_context_lock);
	return job_gres->node_gpu_cnt;
}
extern void gres_plutin_job_gres_init(List gres_list,int cnt)
{
	int i;
	ListIterator gres_iter;
	gres_state_t *gres_ptr;
	...
	if(job_gres->nodebitmap)
		bit_free(job_gres->nodebitmap);
	job_gres->nodebitmap = bit_alloc(cnt);
	job_gres->node_cnt = cnt;
	if(job_gres->slurm_addr)
		xfree(job_gres->slurm_addr);
	job_gres->slurm_addr = xmalloc(sizeof(slurm_addr_t) * cnt);
	if(job_gres->gres_bit_alloc)
		xfree(job_gres->gres_bit_alloc);
	job_gres->gres_bit_alloc = xmalloc(sizeof(bitstr_t *) * cnt);
	if(job_gres->core_cnt)
		xfree(job_gres->core_cnt);
	job_gres->core_cnt=xmalloc(sizeof(uint64_t) * cnt);
	if(job_gres->core_node_slurm_addr)
		xfree(job_gres->core_node_slurm_addr);
	job_gres->core_node_slurm_addr = xmalloc(sizeof(slurm_addr_t) * cnt);
	list_iterator_destroy(gres_iter);
	slurm_mutex_unlock(&gres_context_lock);
	return;
}
extern void gres_plugin_job_alloc_gpu(List job_gres_list,List node_gres_list,int index,uint64_t *job_req_gpu_cnt,slurm_addr_t addr)
{
	int i;
	ListIterator job_gres_iter,  node_gres_iter;
	gres_state_t *job_gres_ptr, *node_gres_ptr;
	...
	while ((job_gres_ptr = (gres_state_t *) list_next(job_gres_iter)))
	{
		...
		if(node_gres_ptr == NULL)
			continue;
		job_gres = (gres_job_state_t *)(job_gres_ptr->gres_data);
		node_gres = (gres_node_state_t *)(node_gres_ptr->gres_data);
	}
	if(job_gres->gres_bit_alloc[index])
		bit_free(job_gres->gres_bit_alloc[index]);
	job_gres->gres_bit_alloc[index] = bit_alloc(node_gres->gres_cnt_avail);
	for(i = 0;i < node_gres->gres_cnt_avail && (*job_req_gpu_cnt) > 0;i ++)
	{
		if(bit_test(node_gres->gres_bit_alloc,i))
			continue;
		bit_set(job_gres->gres_bit_alloc[index],i);
		bit_set(node_gres->gres_bit_alloc,i);
		node_gres->gres_cnt_alloc ++;
		(*job_req_gpu_cnt) --;
	}
	bit_set(job_gres->nodebitmap,index);
	memcpy(&job_gres->slurm_addr[index],&addr,sizeof(slurm_addr_t));
	list_iterator_destroy(job_gres_iter);
	//list_iterator_destroy(node_gres_iter);
	slurm_mutex_unlock(&gres_context_lock);
	return;
}
extern void gres_plugin_job_set_core_info(List gres_list,int index,uint16_t core_cnt,slurm_addr_t addr)
{
	int i;
	ListIterator gres_iter;
	gres_state_t *gres_ptr;
	...
	job_gres->core_cnt[index] = core_cnt;
	memcpy(&job_gres->core_node_slurm_addr[index],&addr,sizeof(slurm_addr_t));
	list_iterator_destroy(gres_iter);
	slurm_mutex_unlock(&gres_context_lock);
	return;
}
extern bitstr_t* gres_plugin_job_get_nodebitmap(List gres_list)
{
	int i;
	ListIterator gres_iter;
	gres_state_t *gres_ptr;
	...
	list_iterator_destroy(gres_iter);
	slurm_mutex_unlock(&gres_context_lock);
	return job_gres->nodebitmap;
}
extern slurm_addr_t* gres_plugin_job_get_slurm_addr(List gres_list)
{
	int i;
	ListIterator gres_iter;
	gres_state_t *gres_ptr;
	...
	list_iterator_destroy(gres_iter);
	slurm_mutex_unlock(&gres_context_lock);
	return job_gres->slurm_addr;
}
extern uint32_t gres_plugin_job_get_node_cnt(List gres_list)
{
	int i;
	ListIterator gres_iter;
	gres_state_t *gres_ptr;
	...
	list_iterator_destroy(gres_iter);
	slurm_mutex_unlock(&gres_context_lock);
	return job_gres->node_cnt;
}
extern bitstr_t** gres_plugin_job_get_gres_bit_alloc(List gres_list)
{
	int i;
	ListIterator gres_iter;
	gres_state_t *gres_ptr;
	...
	list_iterator_destroy(gres_iter);
	slurm_mutex_unlock(&gres_context_lock);
	return job_gres->gres_bit_alloc;
}
extern uint16_t* gres_plugin_job_get_core_cnt(List gres_list)
{
	int i;
	ListIterator gres_iter;
	gres_state_t *gres_ptr;
	...
	list_iterator_destroy(gres_iter);
	slurm_mutex_unlock(&gres_context_lock);
	return job_gres->core_cnt;
}
extern slurm_addr_t* gres_plugin_job_get_core_node_slurm_addr(List gres_list)
{
	int i;
	ListIterator gres_iter;
	gres_state_t *gres_ptr;
	...
	list_iterator_destroy(gres_iter);
	slurm_mutex_unlock(&gres_context_lock);
	return job_gres->core_node_slurm_addr;
}
