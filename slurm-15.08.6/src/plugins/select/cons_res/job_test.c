...
{
	uint16_t cpus;
	uint32_t avail_mem, req_mem,/* gres_cores, gres_cpus,*/ cpus_per_core;//wx
			      ...
			      bitstr_t *exc_core_bitmap, bool qos_preemptor)
{
	...
	uint32_t i, j, free_mem, /*gres_cpus, gres_cores,*/ min_mem;//wx
			...
			bool qos_preemptor, bool preempt_mode)
{
	...
	int first, last;
	List gres_list;//wx
	uint64_t *gpu_cnt,job_gpu_req_cnt = 0,node_gpu_total_cnt = 0;//wx
	...
	{
		job_gpu_req_cnt = gres_plugin_job_req_gpu_cnt(job_ptr->gres_list);
		gpu_cnt = xmalloc(cr_node_cnt * sizeof(uint64_t));
		for(i = 0;i < cr_node_cnt;i ++)
		{
			if(!(bit_test(node_bitmap,i)))
				continue;
			...
			gpu_cnt[i] = gres_plugin_node_gpu_cnt(gres_list,test_only);
			node_gpu_total_cnt += gpu_cnt[i];
		}
		...
		gres_plugin_build_job_gpu_cnt(job_ptr->gres_list,gpu_cnt,cr_node_cnt);
		xfree(gpu_cnt);
	}
	//wx
