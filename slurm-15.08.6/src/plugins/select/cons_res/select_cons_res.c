...
static int _add_job_to_res(struct job_record *job_ptr, int action)
{
	...
	if(action != 2)
	{
		job_req_gpu_cnt = gres_plugin_job_req_gpu_cnt(job_ptr->gres_list);
		gpu_cnt = gres_plugin_job_get_gpu_cnt(job_ptr->gres_list);
		if(gpu_cnt == NULL && job_req_gpu_cnt > 0)
			return SLURM_ERROR;
		else if(job_req_gpu_cnt > 0)
		{
			gres_plutin_job_gres_init(job_ptr->gres_list,select_node_cnt);
			for(i = 0;i < select_node_cnt;i ++)
			{
				if(job_req_gpu_cnt <= 0)
					break;
				...
				if(gpu_cnt[i] > 0)
				{
					...
					if (select_node_usage[i].gres_list)
						...
						gres_list = node_ptr->gres_list;
					gres_plugin_job_alloc_gpu(job_ptr->gres_list,gres_list,i,&job_req_gpu_cnt,node_ptr->slurm_addr);
				}
			}
			for (i = 0, n = -1; i < select_node_cnt; i++)
			{
				if (bit_test(job->node_bitmap, i)) {
					...
					node_ptr = select_node_record[i].node_ptr;
					gres_plugin_job_set_core_info(job_ptr->gres_list,i,job->cpus[n],node_ptr->slurm_addr);
				}
			}
		}
	}
			    ...
			    struct job_record *job_ptr, int action)
{
	...
	if(action != 2)
	{
		gpunodebitmap = gres_plugin_job_get_nodebitmap(job_ptr->gres_list);
		if(gpunodebitmap)
		{
			first_bit = bit_ffs(gpunodebitmap);
			if (first_bit == -1)
				last_bit = -2;
			else
				last_bit =  bit_fls(gpunodebitmap);
			for(i = first_bit;i <= last_bit;i ++)
			{
				node_ptr = node_record_table_ptr + i;
				if (node_usage[i].gres_list)
					...
					gres_list = node_ptr->gres_list;
				gres_plugin_job_dealloc(job_ptr->gres_list,gres_list,i,job_ptr->job_id,node_ptr->name);
			}
		}
	}
	//wx
